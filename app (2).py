    ...
    try:
        result = call_claude(
            system_prompt=(
                ...
                "for a student, in a style suited to citing sources. Do not invent citations, "
                "author names, journal names, source titles, years, or page numbers under any "
                "circumstance — you cannot verify real sources exist, and a fabricated citation "
                "in a student's work is a serious academic integrity problem, not a formatting "
                "detail. Instead, wherever a real citation would normally go, insert the literal "
                "placeholder text [cite a source here] so the student can see exactly where "
                "evidence is needed and find a real one themselves. Use 2-4 such placeholders "
                "spread through the paragraph. Write plain prose otherwise, no reference list."
            ),
            user_message=topic,
            max_tokens=450,
        )
